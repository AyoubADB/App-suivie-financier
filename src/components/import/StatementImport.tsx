import { FileSpreadsheet, Landmark, TriangleAlert, Upload } from 'lucide-react';
...
import { buildDrafts, detectColumns, parseCsv, type ColumnMapping } from '../../logic/csv';
import { formatCents } from '../../logic/money';
import { isOfx, parseOfx } from '../../logic/ofx';
import { applyRulesToDraft } from '../../logic/rules';
import type { ImportDraft, Scope } from '../../types';
import { Modal } from '../ui/Modal';
import { Segmented } from '../ui/Segmented';

/**
 * Un CSV demande un mapping de colonnes, un OFX non : le format porte
 * lui-même le sens de chaque champ. D'où deux formes de fichier lu.
 */
type Parsed =
  | {
      kind: 'csv';
...
/**
 * Import d'un relevé bancaire CSV ou OFX / QFX.
 * Les colonnes d'un CSV sont devinées puis corrigeables : chaque banque
 * exporte à sa façon, et un mapping figé casserait au premier relevé d'une
 * autre banque.
 */
export function StatementImport() {
  ...
  async function onFile(file: File | undefined) {
    ...
    try {
      ...
      if (isOfx(text)) {
        const { drafts, skipped, accountId, currency } = parseOfx(text);
        if (drafts.length === 0) {
          setError("Ce fichier OFX ne contient aucun mouvement lisible.");
          return;
        }
        setParsed({ kind: 'ofx', drafts, skipped, accountId, currency, fileName: file.name, stamp });
        return;
      }
      ...
      setParsed({ kind: 'csv', rows, mapping, hasHeader, fileName: file.name, stamp });
    } catch {
      setError('Lecture impossible. Vérifie que le fichier est bien un CSV ou un OFX.');
    }
  }
  ...
  return (
    <>
      ...
      <Modal open={parsed !== null} onClose={() => setParsed(null)} title="Importer un relevé">
        {parsed && (
          <ImportPreview key={parsed.stamp} parsed={parsed} onDone={() => setParsed(null)} />
        )}
      </Modal>
    </>
  );
...
function ImportPreview({ parsed, onDone }: { parsed: Parsed; onDone: () => void }) {
  const repo = useRepo();
  ...
  const [mapping, setMapping] = useState<ColumnMapping>(
    parsed.kind === 'csv' ? parsed.mapping : { date: -1, label: -1, amount: -1, debit: -1, credit: -1 },
  );
  const [hasHeader, setHasHeader] = useState(parsed.kind === 'csv' ? parsed.hasHeader : false);
  const [scope, setScope] = useState<Scope>(defaultScope === 'pro' ? 'pro' : 'perso');
  const [activityId, setActivityId] = useState('');
  const [useRulesOnImport, setUseRulesOnImport] = useState(true);
  ...
  const { drafts, skipped } = useMemo(() => {
    if (parsed.kind === 'ofx') return { drafts: parsed.drafts, skipped: parsed.skipped };
    return buildDrafts(parsed.rows, mapping, hasHeader);
  }, [parsed, mapping, hasHeader]);

  /**
   * Dédoublonnage. Un identifiant de banque (FITID) tranche sans ambiguïté ;
   * à défaut, on retombe sur la triade date + montant + libellé.
   */
  const seen = useMemo(() => {
    const ids = new Set<string>();
    const triples = new Set<string>();
    for (const tx of existing) {
      if (tx.externalId) ids.add(tx.externalId);
      triples.add(`${tx.date}|${tx.amount}|${normalize(tx.label)}`);
    }
    return { ids, triples };
  }, [existing]);
  ...
  const isDuplicate = useMemo(
    () => (d: ImportDraft) =>
      d.externalId
        ? seen.ids.has(d.externalId)
        : seen.triples.has(`${d.date}|${d.amount}|${normalize(d.label)}`),
    [seen],
  );

  const fresh = useMemo(() => drafts.filter((d) => !isDuplicate(d)), [drafts, isDuplicate]);
  const duplicates = drafts.length - fresh.length;

  async function run() {
    setBusy(true);
    for (const draft of fresh) {
      const suggestion = suggestCategory(draft.label, { scope, type: draft.type, categories });
      const base = {
        type: draft.type,
        scope,
        amount: draft.amount,
      ...
      await repo.addTransaction(useRulesOnImport ? applyRulesToDraft(rules, base) : base);
    }
    setBusy(false);
      ...
      <div className="flex flex-col gap-4">
        <p className="text-sm">
          <span className="font-semibold text-pos">
            {done.added} transaction{done.added > 1 ? 's' : ''} importée{done.added > 1 ? 's' : ''}
          </span>
          {done.duplicates > 0 &&
        ...
        <p className="text-xs text-ink-3">
          Les catégories ont été devinées à partir des libellés{useRulesOnImport && rules.length > 0 ? ', après application de tes règles' : ''}.
          Passe-les en revue dans Mouvements : une correction qui revient souvent mérite une règle.
        </p>
        <button
          type="button"
          onClick={onDone}
    ...
    <div className="flex flex-col gap-4">
      {parsed.kind === 'ofx' ? (
        <p className="flex items-center gap-2 text-xs text-ink-3">
          <Landmark size={14} className="shrink-0 text-accent-2" />
          {parsed.fileName} · format OFX
          {parsed.accountId && ` · compte ${parsed.accountId}`}
      ...
      {rules.length > 0 && (
        <label className="flex items-center gap-2 text-sm text-ink-2">
          <input
            type="checkbox"
            checked={useRulesOnImport}
            onChange={(e) => setUseRulesOnImport(e.target.checked)}
            className="h-4 w-4 accent-[var(--accent)]"
          />
          Appliquer mes {rules.length} règle{rules.length > 1 ? 's' : ''} de catégorisation
        </label>
      )}
...
function PreviewRow({ draft, currency }: { draft: ImportDraft; currency: string }) {
  return (
